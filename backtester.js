...
const BT = (() => {
    ...
    function calcVW(rows) {
        ...
        return rows.reduce((s, r) => s + r._ret * r._size, 0) / total;
    }
        ...
        for (let mi = 0; mi < months.length; mi++) {
            ...
            if (prevLongCodes) {
                ...
                if (strategy === 'long_short' && prevShortCodes) {
                    ...
                    monthTurnover = { ratio: (longTO.ratio + shortTO.ratio) / 2 };
                } else {
                    monthTurnover = longTO;
                }
                totalTO += monthTurnover.ratio;
                toCount++;
            }
    ...
    function computeAllBenchmarks(months) {
        ...
        // Back-fill IR into portfolio metrics
        portfolios.forEach(p => {
            if (!p.results) return;
