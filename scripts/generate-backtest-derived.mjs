...
const workerRuntimeModulePath = path.join(process.cwd(), WORKER_RUNTIME_MODULE_FILE);
mkdirSync(path.dirname(workerRuntimeModulePath), { recursive: true });
writeFileSync(
  workerRuntimeModulePath,
  `// Generated by scripts/generate-backtest-derived.mjs. Do not edit by hand.\nexport default ${JSON.stringify(runtimeData)};\n`,
);
...
if (!existsSync(workerRuntimeModulePath)) {
  throw new Error(`Failed to generate ${WORKER_RUNTIME_MODULE_FILE}`);
}
