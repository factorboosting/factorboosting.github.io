-- Add explicit RMW/CMA 2x3 portfolio buckets and use them for official
-- Profitability/Investment factor construction.

alter table public.factor_panel
  add column if not exists rmw_portfolio text,
  add column if not exists cma_portfolio text;

create index if not exists factor_panel_rmw_portfolio_idx
  on public.factor_panel (universe, month, rmw_portfolio)
  where rmw_portfolio is not null and rmw_portfolio <> '';

create index if not exists factor_panel_cma_portfolio_idx
  on public.factor_panel (universe, month, cma_portfolio)
  where cma_portfolio is not null and cma_portfolio <> '';
...
declare
  ...
  v_op_labels text[];
  v_inv_labels text[];
  v_rmw_portfolios text[];
  v_cma_portfolios text[];
  v_mom_labels text[];
  v_at_labels text[];
  v_sg_labels text[];
  v_acc_labels text[];
...
begin
  ...
  if p_filters ? 'RMW_Portfolio' then v_rmw_portfolios := array(select jsonb_array_elements_text(p_filters -> 'RMW_Portfolio')); end if;
  if p_filters ? 'CMA_Portfolio' then v_cma_portfolios := array(select jsonb_array_elements_text(p_filters -> 'CMA_Portfolio')); end if;
  ...
  if array_length(v_rmw_portfolios, 1) > 0 then v_sql := v_sql || ' and p.rmw_portfolio = any ($9)'; end if;
  if array_length(v_cma_portfolios, 1) > 0 then v_sql := v_sql || ' and p.cma_portfolio = any ($10)'; end if;
  if array_length(v_mom_labels, 1) > 0 then v_sql := v_sql || ' and p.mom_label = any ($11)'; end if;
  if array_length(v_at_labels, 1) > 0 then v_sql := v_sql || ' and p.at_label = any ($12)'; end if;
  if array_length(v_sg_labels, 1) > 0 then v_sql := v_sql || ' and p.sg_label = any ($13)'; end if;
  if array_length(v_acc_labels, 1) > 0 then v_sql := v_sql || ' and p.acc_label = any ($14)'; end if;
  if array_length(v_vol_labels, 1) > 0 then v_sql := v_sql || ' and p.vol_label = any ($15)'; end if;
  if array_length(v_str_labels, 1) > 0 then v_sql := v_sql || ' and p.str_label = any ($16)'; end if;
  ...
  return query execute v_sql using
    ...
    v_bm_labels, v_op_labels, v_inv_labels, v_rmw_portfolios, v_cma_portfolios,
    v_mom_labels, v_at_labels, v_sg_labels, v_acc_labels, v_vol_labels, v_str_labels;
end;
$$;
...
as $$
  with base as (
    ...
    where p.universe = p_universe
      ...
      and (not (p_filters ? 'RMW_Portfolio') or p.rmw_portfolio = any (array(select jsonb_array_elements_text(p_filters -> 'RMW_Portfolio'))))
      and (not (p_filters ? 'CMA_Portfolio') or p.cma_portfolio = any (array(select jsonb_array_elements_text(p_filters -> 'CMA_Portfolio'))))
