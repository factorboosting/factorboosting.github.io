...
function populateTable(tableId, data) {
  ...
  data.forEach((pub) => {
    ...
    const linksHtml = Object.entries(pub.links)
      .map(
        ([key, url]) =>
          `<a href="${url}" class="link">${key.charAt(0).toUpperCase() + key.slice(1)}</a>`,
      )
      .join(" • ");
    ...
    tabBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (section.id === "publications") {
          ...
          const tableId =
            tabName === "all" ? "publicationsTable" : `${tabName}Table`;
...
function downloadFile(filename) {
  ...
  link.href = remoteBacktestFiles.has(filename)
...
function copyCitation() {
  const citation =
    "Foujdar, A., Juneja, S., Kumar, A., Prabhala, N., & Wagle, S. (2025). Portfolio optimization using anomalies: A deep learning approach. Working paper (Submitted).";
