...
function computePortfolio(data, config, months, transactionCost, options = {}) {
  ...
  for (let monthIndex = 0; monthIndex < months.length; monthIndex++) {
    ...
    if (finalLongDF.length >= minFirms) {
      longEW = calcEW(finalLongDF);
      longVW = calcVW(finalLongDF);
      addWeights(finalLongDF, 1.0, 1.0);
    }
    
    if (strategy === "long_short" && finalShortDF.length >= minFirms) {
      shortEW = calcEW(finalShortDF);
      shortVW = calcVW(finalShortDF);
      addWeights(finalShortDF, -1.0, -1.0);
    }
